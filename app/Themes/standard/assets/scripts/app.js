...
class FluteApp {
    ...
    initAuthCheck() {
        ...
        if (this.authToken) {
            this.authCheckInterval = setInterval(() => this.checkAuthStatus(), 10000);
    ...
    setupHtmxEvents() {
        ...
        // Main HTMX events
        htmx.on('htmx:afterSettle', () => this.notifications.updateNotificationCount());
        htmx.on('htmx:afterRequest', (evt) => this.notifications.handleToasts(evt));
    ...
    updateNotificationCount() {
        ...
        if (notificationCount) {
            ...
            notificationCount.style.display =
                count === '' || count === '0' ? 'none' : 'inline-flex';
        }
    }
            ...
            for (const mutation of mutations) {
                ...
                if (mutation.type === 'childList') {
                    ...
                    elementRemoved = removed.some(node => {
                        ...
                        if (node.nodeType === 1 && node.contains(this.activeElement)) return true;
                        return false;
                    });
                    ...
                    if (this.activeElement) {
                        const tooltipText = this.activeElement.getAttribute('data-tooltip');

                        // Если tooltip - это селектор
                        if (tooltipText && tooltipText.startsWith('#')) {
                            try {
                                const tooltipContentEl = document.querySelector(tooltipText);
                                if (tooltipContentEl) {
                                    for (const mutatedNode of mutation.addedNodes) {
                                        if (tooltipContentEl.contains(mutatedNode) || mutatedNode.contains(tooltipContentEl)) {
                                            contentChanged = true;
                                            break;
                                        }
                                    }
                                    ...
                                    for (const mutatedNode of mutation.removedNodes) {
                                        if (tooltipContentEl.contains(mutatedNode) ||
                                            (mutatedNode.nodeType === 1 && mutatedNode.contains(tooltipContentEl))) {
                                            contentChanged = true;
                                            break;
                                        }
                                    }
                    ...
                    if (mutation.target === this.activeElement && mutation.attributeName === 'data-tooltip') {
                        contentChanged = true;
                    }

                    if ((this.activeElement === mutation.target || mutation.target.contains(this.activeElement)) &&
                        (mutation.attributeName === 'style' ||
                            mutation.attributeName === 'class' ||
                            mutation.attributeName === 'hidden')) {

                        const isVisible = this.isElementVisible(this.activeElement);
                        if (!isVisible) {
                            this.hideAllTooltips();
                            break;
                        }
...
class DropdownManager {
    ...
    initDropdownEvents() {
        ...
        $(document).on('mouseenter', '[data-dropdown-open][data-dropdown-hover="true"]', (event) => {
            const $toggle = $(event.currentTarget);
        ...
        $(document).on('click', (event) => {
            ...
            if ($target.closest('[data-dropdown-open]').length ||
        ...
        $(document).on('click', '[data-dropdown] a, [data-dropdown] [data-handler]', (event) => {
...
class ConfirmationManager {
    ...
    initConfirmEvents() {
        ...
        $(document).on('click', '[hx-flute-confirm]', (event) => {
            ...
            const confirmType = $triggerElement.attr('hx-flute-confirm-type') || 'error';
            const actionKey = $triggerElement.attr('hx-flute-action-key');
            const withoutTrigger = $triggerElement.attr('hx-flute-without-trigger');
    ...
    showConfirmDialog(options) {
        ...
        const currentType = this.confirmTypes[type] || this.confirmTypes['error'];
        ...
        $confirmButton.removeClass('btn-accent btn-primary btn-error btn-warning btn-info');
    ...
    handleYoyoConfirmation(yoyoComponent, action, actionKey, originalRequestData) {
            ...
            if (targetSelector) {
                ...
                xhr.onload = () => {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        if (xhr.getAllResponseHeaders().indexOf('hx-trigger') !== -1) {
                        ...
                        const browserEventsHeader = xhr.getResponseHeader('yoyo-browser-event');
