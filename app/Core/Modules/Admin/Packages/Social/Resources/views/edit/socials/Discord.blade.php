...
<x-forms.field>
    ...
    <x-fields.input name="settings__scope" id="settings__scope"
        value="{{ request()->input('settings__scope', $social ? ($social->getSettings()['scope'] ?? 'identify email') : 'identify email') }}"
...
<x-forms.field>
    ...
    <x-fields.input name="settings__token" id="settings__token"
        ...
        placeholder="{{ __('admin-social.edit.discord_token_placeholder') }}" type="password" />
...
<x-forms.field>
        ...
        <x-card :withoutPadding="true">
            <div class="table-responsive">
                <table class="table table-minimal">
                    <thead>
                        <tr>
                            <th style="width: 45%">{{ __('admin-social.edit.discord_roles_map_table.flute_role') }}</th>
                            <th>
                                <div class="d-flex align-items-center gap-1">
                                    <x-icon path="ph.regular.arrow-right" />
                                    {{ __('admin-social.edit.discord_roles_map_table.discord_role_id') }}
                                </div>
                            </th>
                        </tr>
                    </thead>
                            ...
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        <span class="badge" style="background-color: {{ $role->color ?? '#6b7280' }}">
                                            {{ $role->id }}
                                        </span>
                                        <span>{{ $role->name }}</span>
                                    </div>
                                ...
                                <td>
                                    <x-fields.input
                                        name="settings__roles_map[{{ $role->id }}]"
                                        id="rm_{{ $role->id }}"
                                        type="text"
                                        :withoutBottom="true"
    ...
    @else
        <div class="table-empty-state">
            ...
            <p class="table-empty-state__title">{{ __('admin-social.edit.discord_roles_map_empty') }}</p>
        </div>
    @endif

    <small class="text-muted mt-2 d-block">{!! __('admin-social.edit.discord_roles_map_help') !!}</small>
</x-forms.field>
