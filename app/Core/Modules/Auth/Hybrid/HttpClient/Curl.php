 ...
 *
 * Flute: fork of src/HttpClient/Curl.php — TLS verification enabled, insecure curl options
 * rejected (mitigates CVE-2026-4587). Loaded via composer PSR-4 instead of vendor file.
 */
...
class Curl implements HttpClientInterface
{
    ...
    public function request($uri, $method = 'GET', $parameters = [], $headers = [], $multipart = false)
    {
        ...
        if ($this->logger) {
            // phpcs:ignore
            $this->logger->debug(
                sprintf('%s::request( %s, %s ), response:', get_class($this), $uri, $method),
                $this->getResponse(),
            );

            if (false === $response) {
                // phpcs:ignore
                $this->logger->error(sprintf('%s::request( %s, %s ), error:', get_class($this), $uri, $method), [
    ...
    public function setCurlOptions($curlOptions)
    {
        foreach ($curlOptions as $opt => $value) {
            if ($this->isInsecureSslCurlOption((int) $opt, $value)) {
                continue;
            }
    ...
    /**
     * Reject curl options that disable TLS certificate verification (CVE-2026-4587).
     *
     * @param mixed $value
     */
    private function isInsecureSslCurlOption(int $opt, $value): bool
    {
        if ($opt === CURLOPT_SSL_VERIFYPEER) {
            return $value === false || $value === 0;
        }
        if ($opt === CURLOPT_SSL_VERIFYHOST) {
